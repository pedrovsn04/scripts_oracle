...
--          dbms_rowid.rowid_create(1,s2.ROW_WAIT_OBJ#,s2.ROW_WAIT_FILE#,s2.ROW_WAIT_BLOCK#,s2.ROW_WAIT_ROW# ) "Rowid da Linha",
   ...
   ORDER BY 7;
               ...
               SELECT /*+ RULE */
                      ...
                      'Sql Locando -> '||s2.sid||' - '||s2.username||' - '||case when s2.sql_hash_value = 0 then (select ' VERIFICAR NA CURSOR ' FROM DUAL ) else (select sql_text from v$sqlarea where hash_value = s2.sql_hash_value) end " Sql Text - Lock " 
                 FROM sys.dba_lock w,
                      sys.dba_lock h,
                      gv$session s1,
                      gv$session s2
